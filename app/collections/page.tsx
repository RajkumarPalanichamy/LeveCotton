...
export default function CollectionsPage() {
  const { products, loading, error } = useProducts('collections', 1000);
  ...
  useEffect(() => {
    ...
    if (filters.priceRange) {
      const [min, max] = filters.priceRange.split('-').map((v) => (v === '+' ? Infinity : parseInt(v, 10)));
  ...
  const displayProducts =
    filteredProducts.length > 0 || filters.colors.length > 0 || filters.fabrics.length > 0 || filters.priceRange
      ? filteredProducts
      : products;
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-serif text-gray-900 mb-4">Collections</h1>
          <p className="text-gray-600">Explore our curated designer collections</p>
        </div>
